...
function getFileName(_paths: PathItem[]): string { return 'logo.svg'; }
...
export default function ExportModal({ paths, anim, fileName, onClose }: Props) {
  ...
  const visible = paths.filter((p) => p.visible);
  const code = tab === 'css' ? generateCSS(paths, anim) : tab === 'python' ? generatePython(paths, anim, fileName) : generateLottie(paths, anim);
  const highlighted = tab === 'python' ? tokenizePython(code) : tab === 'css' ? tokenizeCSS(code) : esc(code);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tabs */}
        <div style={{ padding: '12px 24px 0', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
          ...
          {COMING_SOON.map((f) => (
            <div key={f} style={{ padding: '6px 12px', borderRadius: 8, fontSize: 12.5, color: 'rgba(240,238,255,0.2)', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
              {f}
              <span style={{ fontSize: 9, padding: '1px 5px', background: 'rgba(255,255,255,0.06)', borderRadius: 4, color: 'rgba(240,238,255,0.25)', textTransform: 'uppercase', letterSpacing: 0.8 }}>soon</span>
            </div>
          ))}
        </div>
        ...
        {/* Footer */}
        <div style={{ padding: '14px 24px', borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8 }}>
